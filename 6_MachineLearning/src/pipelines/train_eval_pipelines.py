"""
Contain full training+evalution pipelines to simplify code and improve readability.
"""
...
from .model_training import (
    pipeline_gridsearch_2d_with_loocv,
    pipeline_gridsearch_3d_with_loocv,
    classifiers
)
from ..pipelines.shap_feature_importance import pipeline_feature_importance
from ..feature_extraction.mrmr_feature_selection import mrmr_feature_selection
...
def train_eval_gridsearch_2d_with_loocv(
    ...
    grid_search_params = {  # We add the "NF" and useMRMR params
        "NF": config_params["NF"],
    ...
    best_hyperparams, best_score, _ = pipeline_gridsearch_2d_with_loocv(
    ...
    if "NF" in best_hyperparams:  # The args model will not contain "NF"
        args_model = {
            key: value for key, value in best_hyperparams.items() if key != "NF"
        }
    else:
        args_model = best_hyperparams
...
def train_eval_gridsearch_loocv_with_outer_kfold_loop(
    x, y, feature_columns, config_params, outputs_folder, which_gridsearch="2D", k=5
):
    """
    This function runs an outer K-fold cross validation approach to the 2D Gridsearch
    with LOOCV approach, now generalized to several classifiers and allowing for
    flexibility in the hyperparameter choice.
    ...
    For this reason, we perform an outer loop with K-fold, running the KNN GridSearch +
    LOOCV entire algorithm for each train/test fold in the K-fold split.

    We will finally report the average of the test metrics over the K partitions.

    Technical consideration: We will make K calls to the
    train_eval_gridsearch_2d_with_loocv function, each time storing results in a
    different folder.
    ...
    Args:
        ...
        which_gridsearch: Which gridsearch to apply, either "2D" or "3D" (with NF)
        k: (int) number of partitions for the outer K-fold cross validation loop.

    Returns:

    """
    # We first perform the K-fold outer loop split. Since we have unbalanced
    # datasets, we will make use of the Stratified KFold.
    skfold = StratifiedKFold(
        n_splits=k, shuffle=True, random_state=config_params["RANDOM_STATE"]
    )

    if which_gridsearch == "2D":
        train_eval_function = train_eval_gridsearch_2d_with_loocv
    elif which_gridsearch == "3D":
        train_eval_function = train_eval_gridsearch_3d_with_loocv
    else:
        raise ValueError("which_gridsearch parameter must be either '2D' or '3D'")

    for i, (train_index, test_index) in enumerate(skfold.split(x, y)):
        print(f"\n -------- Running GridSearch  Pipeline for Fold {i} -------- \n")
    ...
    print("Finished Running KNN GridSearch pipeline for all folds")
...
def train_eval_gridsearch_loocv_with_outer_n_loop(
    x, y, feature_columns, config_params, outputs_folder, which_gridsearch="2D", n=20
):
    """
    This function runs an N iterations of the 2D Gridsearch with LOOCV approach,
